...
export default class RideScreen extends Component {
    ...
    handleTransaction = async () => {
        ...
        if (!transactionType) {
            ...
            // Verifique a disponibilidade do usuário para encerrar o passeio 
        ...
        if (bikeRef.docs.length == 0) {
            ...
            bikeRef.docs.map(doc => {
                if (!doc.data().under_maintenance) {
                    //se a bicicleta estiver disponível, o tipo de transação será 'rented' (alugado), caso contrário, será 'return' (devolver)
    ...
    returnBike = async (bikeId, userId, bikeType, userName) => {
        ...
        //alterar valor da bicicleta atribuída ao usuário
        db.collection("users")
            .doc(userId)
            .update({
                bike_assigned: false
